/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   THIS IS NOT SECURE AUTHENTICATION AND MUST NOT BE PRESENTED AS SUCH.
   ...
   signed session and no expiry beyond a timestamp this same client wrote. It
   exists so the portal can be demonstrated and reviewed without a backend.

   When a real backend lands, replace signIn/signOut with calls to it and store
   the returned token instead of this flag. The portal reads auth only through
...
const SESSION_KEY = "hf_admin_demo_session_v1";

/** Eight hours — a convenience for the demo, not a security control. */
const SESSION_MS = 8 * 60 * 60 * 1000;

export type AdminSession = { email: string; signedInAt: number };
...
/* ---------------------------------------------------------------- store
   ...
   hydration, and a mount effect would cost a cascading render. */

const listeners = new Set<() => void>();

/* getSnapshot must stay referentially stable between changes, so the parsed
...
/** The server has no session — signed out until the client hydrates. */
